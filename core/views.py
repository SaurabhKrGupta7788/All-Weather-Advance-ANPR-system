...
pipeline = WeatherRestorationPipeline(
    ...
    label_name="clean"  # Change as needed for testing (e.g., "rainy", "hazy", "clear"
)
...
################################################333333333333333333333333333333333
        ...
        # frame = smogger.apply(frame)   ##### no
        
        # frame = cv2.resize(frame,(256,256))

        # frame, weather = pipeline(frame)

        # frame = np.ascontiguousarray(frame)

        # frame = model_naf(frame)     # deblurring before detection can help in some cases
...
#####################################################################################
                ...
                SAVE_DIR = "debug_vehicle_crops"
                os.makedirs(SAVE_DIR, exist_ok=True)

                debug_path = os.path.join(SAVE_DIR, f"vehicle_crop{track_id}.jpg")
                ...
                debug_path = os.path.join(SAVE_DIR, f"vehicle_crop_blur_done{track_id}.jpg")
                cv2.imwrite(debug_path, vehicle_crop)
                ...
                for pr in plate_results:
                    for pbox in pr.boxes:
                        ...
                        # plate_crop = apply_blur(plate_crop)



                        # SAVE_DIR = "debug_plate_crops"
                        # os.makedirs(SAVE_DIR, exist_ok=True)

                        # debug_path = os.path.join(SAVE_DIR, f"plate_crop{track_id}.jpg")
                        # cv2.imwrite(debug_path, plate_crop)

                        #plate_crop = esrgan_super_resolve_cv(plate_crop)

                        

                        # plate_crop = cv2.resize(plate_crop, (plate_crop.shape[1]*2, plate_crop.shape[0]*2), interpolation=cv2.INTER_CUBIC)

                        

                        # plate_crop = restore_image(plate_crop, model = model_deblur)
